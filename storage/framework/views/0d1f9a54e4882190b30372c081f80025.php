...
<div x-data="{ openModal: false, imgModal: '', referenciaModal: '' }" class="flex flex-wrap gap-2 w-full border-4 md:border-8 border-gray-300 dark:border-slate-900 bg-white dark:bg-slate-900  max-h-96 soft-scrollbar overflow-y-auto rounded-md p-1">
    ...
    <div class="w-auto bg-white  rounded-md border-4 border-gary-300 dark:border-slate-700">
        <img
        ...
        class="w-24 min-h-24 max-h-24 rounded-md mx-auto cursor-pointer" src="<?php echo e($item->imagem_cliente ? url("storage/produtos/imagens_pedidos/{$item->imagem_cliente}") : url("storage/{$item->produtos->imagem}")); ?> " alt="transfer">  
    

        <!--Input quantidade inicio -->
        <div class="p-1 mt-1 bg-gray-200 dark:bg-slate-700 text-center">
        ...
        <div x-data="{ quantity: <?php if ((object) ('quantities.' . $item->produto->id) instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('quantities.' . $item->produto->id->value()); ?>')<?php echo e('quantities.' . $item->produto->id->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('quantities.' . $item->produto->id); ?>')<?php endif; ?> }" class="flex justify-center  md:gap-x-1">
            ...
            <button @click="quantity = Math.max(0, quantity - 1)" id="subqnde" type="button" class="px-2 py-1 text-sm font-medium text-center inline-flex items-center text-white bg-gray-100 rounded-lg hover:bg-gray-300 focus:ring-4 focus:outline-none focus:ring-gray-300 dark:bg-slate-600 dark:hover:bg-slate-700 dark:focus:ring-slate-800">
            ...
            <div class="w-16 text-center">
                <input
                    ...
                    class="block w-16 p-1 md:p-2 text-center text-blue-600 border border-slate-300 rounded-lg bg-gray-100 sm:text-xs focus:ring-slate-500 focus:border-slate-500 dark:bg-slate-700 dark:border-slate-600 dark:placeholder-slate-400 dark:text-white dark:focus:ring-slate-500 dark:focus:border-slate-500"
                    placeholder="000"
                    required>
            </div>
            
            <button @click="quantity = quantity + 1" id="addqnde" type="button" class="px-2 py-1 text-sm font-medium text-center inline-flex items-center text-white bg-gray-100 rounded-lg hover:bg-gray-300 focus:ring-4 focus:outline-none focus:ring-gray-300 dark:bg-slate-600 dark:hover:bg-slate-700 dark:focus:ring-slate-800">
                <svg class="w-4 h-4 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    <div x-show="openModal" @click.away="openModal = false" class="fixed inset-0 flex items-center justify-center z-50">
        ...
        <div @click.away="openModal = false" x-show="openModal" class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full p-2">
            <div class="m-auto">
                <img :src="imgModal" alt="Imagem do modal" class="mx-auto w-full max-h-80 md:max-h-96 md:w-80 rounded-lg" >
            </div>
...
</div><?php /**PATH /home/u637911780/domains/printview.shop/resources/views/components/pedidos/md/editor/itensPedidoMD.blade.php ENDPATH**/ ?>