<div>   
    <x-slot name="header">
        <h2 class="flex gap-x-2 justify-center md:justify-start font-semibold md:text-xl text-orange-600 dark:text-orange-500 leading-tight">
            ...
            {{ __('Permissão acesso sistema - Cliente(s)') }}
        </h2>
    </x-slot>
        ...
        <div class="w-full md:w-10/12 mx-auto sm:px-2 lg:p-3 bg-white dark:bg-slate-800 rounded-lg">
            ...
            <div class="md:flex md:items-end md:space-x-5 space-y-3 bg-slate-300 dark:bg-slate-900 p-2 rounded-lg">
                ...
                <div class="w-full md:w-3/12">                        
                    <x-select autocomplete="off" z-index="z-50" label="Encontre o cliente que deseja atribuir permissão" placeholder="Selecione um usuário"
                ...
                <form wire:submit="liberarCliente" class="md:flex items-end gap-x-5">
                    <div class="flex gap-x-3 mb-3">
                        {{-- <x-checkbox-wire  wire:model="checkboxPadrao"  positive id="right-label" label="LIBERAR PERMISSÕES PADRÃO ( CLIENTE )" /> --}}
                        <x-checkbox-wire  wire:model.live="checkboxLiberarMarcas"  positive id="left-label" label="LIBERAR TODAS MARCA(S)" />
                        <x-checkbox-wire  wire:model.live="checkboxLiberarSemMarcas"  positive id="right-label" label="LIBERAR APENAS MARCA (037)" />
                    </div>

                    <div>
                        @if($checkboxLiberarMarcas)
                        <x-button-wire type="submit" wire:loading.attr="disabled" sky  icon="check" label="Liberar Cliente + Todas Marca(s)" spinner  class="w-full mt-3"/>
                        @elseif($checkboxLiberarSemMarcas)
                        <x-button-wire type="submit" wire:loading.attr="disabled" sky  icon="check" label="Liberar Cliente + Marca (037)" spinner  class="w-full mt-3"/>  
                        @else
                            <x-button-wire type="submit" wire:loading.attr="disabled" teal icon="check" label="Liberar Cliente" spinner  class="w-full mt-3"/>
                        @endif               
                    </div>
                </form>
            ...
            <div class="md:flex mt-5 bg-white dark:bg-slate-800 overflow-hidden shadow-xl rounded-lg">
                ...
                <div class="bg-blue-400 md:w-6/12 rounded-lg">
                    ...
                    <div class="p-2">
                        <x-input-wire wire:model.live="search" icon="search" placeholder="pesquise a permissão..."/>
                        <!--div table -->
                ...
                <div class="bg-green-400 md:w-6/12 rounded-lg">
                    ...
                    <div class="p-2">
                        <x-input-wire wire:model.live="searchAssociados" icon="search" placeholder="pesquise a permissão..."/>
                         ...
                         <div class="mt-2 max-h-80 md:max-h-50v relative overflow-x-hidden overflow-y-auto soft-scrollbar shadow-md sm:rounded-lg">
                            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    @forelse ($regrasAssociadasAoUsuario as $associadas)
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        <td class="w-4 p-4">
                                            <div class="flex items-center">
                                        ...
                                        <td class="flex items-center px-6 py-4">                       
                                            <a wire:click="removeRegraUser({{ $associadas->id }})" wire:key="{{ $associadas->id }}" class="font-medium text-red-600 dark:text-red-400 hover:underline ms-3 cursor-pointer">Remover</a>
                                        </td>
                                    </tr>
