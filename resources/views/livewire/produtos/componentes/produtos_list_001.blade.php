<div>
    ...
    <div x-data="{ value: 12 }"
        ...
        <div class="flex justify-between dark:bg-slate-900 mb-4 p-1 rounded-lg">
            ...
            <x-button-wire x-on:click="$openModal('simpleModal')" icon="adjustments" positive label="Filtrar por"
                                ...
                                <div class="w-full">
                                    <x-select autocomplete="false" tabindex="-1"
                                        ...
                                        @foreach ($filtroList as $filter)
                                            <x-select.option label="{{ $filter->filtro }}"
                                                wire:key="{{ $filter->id }}" value="{{ $filter->filtro }}" />
                                        @endforeach
                                    </x-select>
                                </div>
        ...
        <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-12 content-start px-1 gap-1 md:gap-2 mb-0 md:mb-3 rounded-lg  overflow-y-auto soft-scrollbar max-h-70v md:max-h-70v"
            ...
            {{-- <div
            class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-12 xl:grid-cols-12 content-start px-1 gap-1 md:gap-2 mb-0 md:mb-3 rounded-lg  overflow-y-auto soft-scrollbar max-h-70v md:max-h-70v"> --}}

            @forelse ($produtoList as $transfer)
                @if (!in_array($transfer->id, $carrinho))
                    <div x-data="{ showModal: false }" wire:key="{{ $transfer->id }}"
                        :class="{
                        ...
                        class="{{-- col-span-1 h-40 md:h-48  --}} {{-- flex flex-col items-center --}} rounded-lg bg-white dark:bg-slate-700 p-1 shadow-lg">
                        ...
                        <div class="rounded-t-lg bg-white flex justify-center items-center">
                            <img @click="showModal = true"
                                ...
                                {{-- src="{{ url("storage/{$transfer->imagem}") }}" --}} alt="Produto sem foto. Avise-nos!"
                                class="{{-- h-img-produto --}} {{-- md:h-36 --}} rounded-t-lg cursor-pointer"
                                oncontextmenu="return false;" />
                        </div>
                        <div class="p-1 text-center ">
                            <div class="flex justify-center w-full clear-left text-center py-1 md:py-2 ">
                                <span
                                    class="font-bold text-sm text-gray-600 dark:text-gray-300">{{ $transfer->referencia }}
                                    </h1>
                            </div>
                            ...
                            <div
                                ...
                                <div @click.away="showModal = false" x-show="showModal"
                                    ...
                                    <div class="p-2 bg-gray-200 rounded-lg">
                                        <h1 class="text-center font-bold text-green-600">{{ $transfer->referencia }}
                                        </h1>
                                    </div>
                                    <div>
                                        <img src="{{ url("storage/{$transfer->imagem}") }}" alt=""
                                    ...
                                    <div class="flex items-center gap-x-2 ml-3">
                                        ...
                                        <h1 class="text-xs text-gray-400 uppercase">{{ $transfer->filtros }}</h1>
                                    </div>

                                    <div class="p-3 w-full ">
                                        <button wire:click="addCarrinho({{ $transfer->id }})"
