...
<div x-data="{ openModal: false, imgModal: '', referenciaModal: '' }" class="flex flex-wrap gap-2 w-full border-4 md:border-8 border-gray-300 dark:border-slate-900 bg-white dark:bg-slate-900  max-h-96 soft-scrollbar overflow-y-auto rounded-md p-1">
    ...
    <div class="{{-- col-span-2 --}}w-auto bg-white  rounded-md border-4 border-gary-300 dark:border-slate-700">
        <img
        ...
        class="w-24 min-h-24 max-h-24 rounded-md mx-auto cursor-pointer" src="{{ $item->imagem_cliente ? url("storage/produtos/imagens_pedidos/{$item->imagem_cliente}") : url("storage/{$item->produtos->imagem}") }} " alt="transfer">  
