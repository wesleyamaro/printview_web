...
<div x-data="{ openModal: false, imgModal: '', referenciaModal: '' }" class="flex flex-wrap  gap-2 w-full border-4 md:border-8 border-gray-300 dark:border-slate-900 bg-white dark:bg-slate-900  max-h-96 soft-scrollbar overflow-y-auto rounded-md p-1">
    ...
    <div class="{{-- col-span-2 --}} w-auto bg-gray-50  rounded-md border-8 border-gray-100 dark:border-slate-700 shadow-md">
        ...
        <div class="w-24 min-h-28 max-h-28 rounded-md">
            ...
            class="w-24 min-h-28 max-h-28 mx-auto cursor-pointer" src="{{ $item->imagem_cliente ? url("storage/produtos/imagens_pedidos/{$item->imagem_cliente}") : url("storage/{$item->produtos->imagem}") }} " alt="transfer">  
        </div>
