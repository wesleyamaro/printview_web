<div class="">
    ...
    <div class="p-2 md:p-5 rounded-lg bg-slate-300 dark:bg-slate-700">
        ...
        <div x-data="{ openModal: false, imgModal: '', filtros: '', medidas: '' }" class="relative overflow-y-auto soft-scrollbar shadow-md sm:rounded-lg  max-h-55v">
            <table class="w-full text-left text-gray-500 dark:text-gray-400 rounded-xl">
                ...
                <tbody>
                    <!-- row 1 -->
                    @forelse ($termopatchList as $termo)
                        <tr
                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-900">
                            <td class="uppercase text-center">{{ '#' . $termo->id }}</td>
                            <td class="text-center">
                                <img {{-- wire:click="openModal('{{ url("storage/{$transfer->imagem}") }}')" --}}
                                @click="openModal = true; 
                                imgModal = '{{ url("storage/{$termo->imagem}") }}'; 
                                filtros = '{{ $termo->filtros }}';
                                medidas = '{{ $termo->medidas }}'";
                                    class="cursor-pointer p-1 rounded-lg min-h-20 max-h-20"
                                    src="{{ url("storage/{$termo->imagem}") }}" alt="imagem do produto" />
                            </td>
                            <td class="uppercase text-center">{{ $termo->referencia }}</td>  
                            <td class="uppercase text-center">
                                {{ $termo->marca->referencia . ' / ' . $termo->marca->descricao }}</td>
                            <td class="uppercase text-center">{{ $termo->medidas }}</td>
                            <td class="uppercase text-center">{{ $termo->nomeCliente }}</td>
                            <td class="uppercase text-center">{{ $termo->filtros }}</td>                            
                            <td class="uppercase text-center">{{ $termo->user->name }}</td>
                            <td class="uppercase text-center">{{ $termo->created_at->format('d/m/Y H:i') }}</td>
                            <td class="text-center">
                                <div class="flex justify-center items-center gap-x-5">

                                    @can('EDIT-TERMOPATCH', $permissao)
                                        <a class="text-red-600 cursor-pointer"
                                            wire:click="editTermopatch({{ $termo->id }})">
                                    ...
                                    @can('DEL-TERMOPATCH', $permissao)
                                        <a class="text-red-600 cursor-pointer"
                                            wire:click="confirmDeleteTermopatch({{ $termo->id }})">
    ...
    <div class="flex justify-between mt-2 ">
        ...
        @if ($termopatchList->hasMorePages())
        ...
        <h1 class="font-bold dark:text-gray-400">Total de registros: <span
                class="dark:text-green-400">{{ $termopatchList->count() }}</span></h1>
    </div>
